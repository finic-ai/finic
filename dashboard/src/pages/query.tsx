...
const ProductsTable: FC = function () {
  ...
  const getBotResponse = async (message: string) => {
    ...
    const dedupedSources = sources.filter((source: any, index: number) => {
      return (
        index ===
  ...
  return (
    <>
      <div className="h-[500px] w-full overflow-y-auto bg-gray-200 ">
      ...
      <form onSubmit={handleSubmit} className="border-t border-gray-300 p-4">
        <div className="flex items-center">
          <input
            ...
            disabled={loading}
            className="grow rounded-lg border border-gray-300 px-4 py-2 focus:border-blue-500 focus:outline-none"
            placeholder="Type your message"
          />
          <button
            type="submit"
            disabled={loading}
            className="ml-4 rounded-lg bg-blue-500 px-6 py-2 text-white focus:bg-blue-600 focus:outline-none"
          >
...
const Message: React.FC<MessageProps> = ({ message, isUser }) => (
  <div
    className={`min-h-50 flex items-center px-2 py-8  ${
      isUser ? "bg-white" : "bg-gray-200"
    }`}
  >
    <div className={`mx-4 flex h-10 w-10 items-center justify-center`}>
