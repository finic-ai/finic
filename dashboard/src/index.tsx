  ...
  useEffect(() => {
    supabase.auth.getSession().then(({ data: { session } }: any) => {
      ...
      if (scriptLoaded && (window as any).Intercom) {
        const name = session.user.identities.length > 0 ? session.user.identities[0].identity_data.name : null;
    ...
    return (
      <PostHogProvider client={posthog}>
        <Flowbite theme={{ theme }}>
          <Helmet onChangeClientState={(newState) => handleScriptInject(newState)}>
            <script>
