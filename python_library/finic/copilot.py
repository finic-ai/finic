...
async def handle_inspect_node(cdp_session: CDPSession, page: Page, event: Dict[str, Any], selected_node: List[NodeInfo]):
...
def execute_plan(plan: List[Step], cdp_session: CDPSession, page: Page):
    selected_node: List[NodeInfo] = [None]
    for step in plan:
        plan_string = "\n".join([f"{s.step_number}. {s.description}" for s in plan])
        requested_elements = baml.ElementsPerStep(plan=plan_string, current_step=step)
        if len(requested_elements) > 0:
            # Set up event listener for DOM.inspectNodeRequested
            cdp_session.on("Overlay.inspectNodeRequested", lambda event: handle_inspect_node(cdp_session, page, event, selected_node))
            print("Please select the following element:")
