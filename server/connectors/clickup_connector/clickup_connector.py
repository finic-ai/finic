...
class ClickupConnector(DocumentConnector):
    ...
    async def authorize(self, account_id: str, auth_code: Optional[str], metadata: Optional[Dict]) -> AuthorizationResult:
        ...
        try:
            ...
            response = requests.get(
                f"{BASE_URL}/v2/team", headers=headers)
            ...
            raise Exception(
                f"Unable to load workspaces from Clickup due to error: {e}")
    ...
    async def get_sections(self, account_id: str) -> List[Section]:
        ...
        for team_doc in team_docs:
            team_id = team_doc["id"]
            team_sec = Section(
                id=team_id, name=team_doc["name"]+" [WORKSPACE]", type=SectionType.folder, children=[])
            space_docs = query_clickup(
                f"team/{team_id}/space")["spaces"]
            for space_doc in space_docs:
                space_id = space_doc["id"]
                space_sec = Section(
                    id=space_id, name=space_doc["name"]+" [SPACE]", type=SectionType.folder, children=[])

                # lists without folders
                list_docs = query_clickup(
                    f"space/{space_id}/list")["lists"]
                if len(list_doc) > 0:
                    unfoldered_sec = Section(
                        id=space_id, name=" [UNFOLDERED LISTS]", type=SectionType.folder, children=[])
                for list_doc in list_docs:
                    list_id = list_doc["id"]
                    list_sec = Section(
                        id=list_id, name=list_doc["name"]+" [LIST]", type=SectionType.folder, children=[])
                    unfoldered_sec.children.append(list_sec)

                space_sec.children.append(unfoldered_sec)
                folder_docs = query_clickup(
                    f"space/{space_id}/folder")["folders"]
                for folder_doc in folder_docs:
                    folder_id = folder_doc["id"]
                    folder_sec = Section(
                        id=folder_id, name=folder_doc["name"]+" [FOLDER]", type=SectionType.folder, children=[])

                    list_docs = query_clickup(
                        f"folder/{folder_id}/list")["lists"]
                    for list_doc in list_docs:
                        list_id = list_doc["id"]
                        list_sec = Section(
                            id=list_id, name=list_doc["name"]+" [LIST]", type=SectionType.folder, children=[])
    ...
    async def load(self, connection_filter: ConnectionFilter) -> List[Document]:
        ...
        for (list_id, path) in list_id_path_array:
            ...
            partial_tasks = query_clickup_tasks(
                f"list/{list_id}/task", {"include_closed": True})["tasks"]
    ...
    def _get_all_lists(self, section, path):
        ...
        if section.name.endsWith("[LIST]"):
            ...
            for child_sec in section.children:
                lists.extend(
                    self._get_all_lists(child_sec, path))
        return lists
