  ...
  return (
    <div className="flex flex-col items-center justify-center gap-2">
      ...
      {results && <div className="flex w-112 flex-col items-start gap-6 rounded border border-solid border-neutral-border bg-neutral-100 pt-6 pr-6 pb-6 pl-6 shadow-overlay">
        ...
        <div className="nodrag cursor-default select-text w-full overflow-auto max-h-80" onClick={stopPropagation}>
          <table className="gap-4 w-full table-auto text-left" ref={resultTableRef}>
            <thead>
              <tr>
                {results.columns.map((column, index) => (
                  <th key={index} className="text-heading-3 font-heading-3 text-default-font pr-4">
                    {column}
                  </th>
                ))}
              </tr>
            ...
            <tbody>
              {results.data.map((row, index) => (
                <tr key={index}>
                  {row.map((cell, index) => (
                    <td key={index} className="text-body font-body text-default-font">
                      {cell.toString()}
                    </td>
                  ))}
                </tr>
